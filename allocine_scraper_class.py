...
class AllocineScraper:
    ...
    def collect_review_links(self): 
        for genre_name, genre_id in self.genres_dict.items():
            for nb_page in range(1, self.MAX_PAGES + 1):
                url = f"https://www.allocine.fr/films/genre-{genre_id}/pays-5002/?page={nb_page}"
                try:
                    self.driver.get(url)
    ...
    def scrape_reviews(self, max_reviews: Type[int] = 4000):
        for i, link in enumerate(self.list_href):
            if i >= max_reviews:
                break
